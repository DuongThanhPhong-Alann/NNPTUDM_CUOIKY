...
router.get("/:id", async function (req, res, next) {
  try {
    let item = await canHoImageController.GetById(req.params.id);
...
router.post(
  ...
  async function (req, res, next) {
    try {
      let item = await canHoImageController.Create(buildPayload(req));
      res.send(item);
    } catch (err) {
...
router.put(
  ...
  async function (req, res, next) {
    try {
      let updated = await canHoImageController.UpdateById(req.params.id, buildPayload(req));
