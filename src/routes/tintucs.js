...
let tinTucController = require("../controllers/tintucs");
const { requireAuth, requireRoles } = require("../middlewares/auth");

router.get("/", async function (req, res, next) {
  let items = await tinTucController.GetAll();
  res.send(items);
});

router.get("/:id", async function (req, res, next) {
  try {
    let item = await tinTucController.GetById(req.params.id);
...
router.post("/", requireAuth, requireRoles("Ban quan ly"), async function (req, res, next) {
  try {
    let item = await tinTucController.Create(req.body);
    res.send(item);
  } catch (err) {
...
router.put("/:id", requireAuth, requireRoles("Ban quan ly"), async function (req, res, next) {
  try {
    let updated = await tinTucController.UpdateById(req.params.id, req.body);
...
router.delete("/:id", requireAuth, requireRoles("Ban quan ly"), async function (req, res, next) {
  try {
    let deleted = await tinTucController.DeleteById(req.params.id);
